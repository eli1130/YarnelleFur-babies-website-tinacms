    ...
    collections: [
      {
        ...
        ui: {
          ...
          filename: {
            readonly: false,
            slugify: (values) =>
              `${values?.slug || values?.title?.toLowerCase().replace(/ /g, "-")}`,
          },
        },
        fields: [
          { type: "boolean", name: "active", label: "Active (uncheck to hide this litter)", ui: { defaultValue: true } },
          { type: "boolean", name: "featured", label: "Show on Homepage", ui: { defaultValue: false } },
...

      {
        ...
        ui: {
          filename: {
            readonly: false,
            slugify: (values) =>
              `${values?.slug || values?.breed?.toLowerCase().replace(/ /g, "-") || "upcoming"}`,
          },
        },
        fields: [
